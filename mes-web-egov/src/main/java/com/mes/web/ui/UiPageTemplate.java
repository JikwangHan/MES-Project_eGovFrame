...
public final class UiPageTemplate {
    ...
    public static String render(String title, String heading, String message, String currentPath) {
        ...
        if ("/ui/kpi".equals(currentPath)) {
            ...
            html.append("</div>");
            html.append("<div style=\"display:flex;gap:6px;flex-wrap:wrap;margin-top:8px;\">");
            html.append("<button id=\"kpi-range-today\" style=\"padding:4px 8px;\">오늘</button>");
            html.append("<button id=\"kpi-range-7\" style=\"padding:4px 8px;\">최근 7일</button>");
            html.append("<button id=\"kpi-range-30\" style=\"padding:4px 8px;\">최근 30일</button>");
            html.append("<button id=\"kpi-clear\" style=\"padding:4px 8px;\">기간 초기화</button>");
            html.append("</div>");
            ...
            html.append("<div id=\"kpi-result-summary\" style=\"margin-top:6px;color:#6b7280;\">조회 결과: -</div>");
            html.append("<table style=\"width:100%;border-collapse:collapse;margin-top:8px;\">");
            html.append("<thead><tr>");
        ...
        if ("/ui/kpi".equals(currentPath) || "/ui/orders".equals(currentPath)
            ...
            html.append("// KPI 조회는 빠른 기간 버튼과 결과 요약까지 함께 갱신한다.");
            html.append("var kpiSearch=document.getElementById(\"kpi-search\");");
            html.append("var kpiRangeToday=document.getElementById(\"kpi-range-today\");");
            html.append("var kpiRange7=document.getElementById(\"kpi-range-7\");");
            html.append("var kpiRange30=document.getElementById(\"kpi-range-30\");");
            html.append("var kpiClear=document.getElementById(\"kpi-clear\");");
            html.append("function kpiParams(){");
            html.append("return {");
            ...
            html.append("};");
            html.append("}");
            html.append("function kpiSetResultSummary(count){");
            html.append("var el=document.getElementById(\"kpi-result-summary\");");
            html.append("if(!el){return;}");
            html.append("if(count===undefined||count===null){el.textContent=\"조회 결과: -\";return;}");
            html.append("el.textContent=\"조회 결과: \"+count+\"건\";");
            html.append("}");
            html.append("function kpiUpdateState(){");
            ...
            html.append("setSummary(\"kpi-summary\",kpiParams());");
            html.append("return msg;");
            html.append("}");
            html.append("function kpiFormatDate(date){");
            html.append("var y=date.getFullYear();");
            html.append("var m=String(date.getMonth()+1).padStart(2,\"0\");");
            html.append("var d=String(date.getDate()).padStart(2,\"0\");");
            html.append("return y+\"-\"+m+\"-\"+d;");
            html.append("}");
            html.append("function kpiApplyRange(days){");
            html.append("// 초보자가 빠르게 조회할 수 있도록 버튼으로 기간을 채운다.");
            html.append("var today=new Date();");
            html.append("var start=new Date();");
            html.append("if(days>0){start.setDate(today.getDate()-(days-1));}");
            html.append("document.getElementById(\"kpi-from\").value=days>0?kpiFormatDate(start):\"\";");
            html.append("document.getElementById(\"kpi-to\").value=days>0?kpiFormatDate(today):\"\";");
            html.append("}");
            html.append("function fetchKpiData(params){");
            html.append("// 목록/추이/차트를 동시에 갱신해 화면 상태가 뒤섞이지 않도록 한다.");
            html.append("var q=buildQuery(params);");
            html.append("fetchJson(\"/api/kpi\"+q,function(res){");
            html.append("var rows=(res&&res.data)||[];");
            html.append("fillTable(\"kpi-body\",rows,[\"name\",\"targetValue\",\"currentValue\",\"progressRate\",\"resultValue\",\"unit\"]);");
            html.append("kpiSetResultSummary(rows.length);");
            html.append("},function(err){");
            html.append("setWarning(\"kpi-warning\",err);");
            html.append("kpiSetResultSummary(null);");
            html.append("});");
            ...
            html.append("var el=document.getElementById(id);");
            html.append("if(el){el.addEventListener(\"input\",function(){");
            html.append("kpiUpdateState();");
            html.append("});}");
            html.append("});");
            html.append("kpiUpdateState();");
            html.append("fetchKpiData(kpiParams());");
            html.append("if(kpiSearch){");
            html.append("kpiSearch.addEventListener(\"click\",function(){");
            html.append("var msg=kpiUpdateState();");
            html.append("if(msg){return;}");
            html.append("fetchKpiData(kpiParams());");
            html.append("});");
            html.append("}");
            html.append("function handleRange(days){");
            html.append("kpiApplyRange(days);");
            html.append("var msg=kpiUpdateState();");
            html.append("if(msg){return;}");
            html.append("fetchKpiData(kpiParams());");
            html.append("}");
            html.append("if(kpiRangeToday){kpiRangeToday.addEventListener(\"click\",function(){handleRange(1);});}");
            html.append("if(kpiRange7){kpiRange7.addEventListener(\"click\",function(){handleRange(7);});}");
            html.append("if(kpiRange30){kpiRange30.addEventListener(\"click\",function(){handleRange(30);});}");
            html.append("if(kpiClear){");
            html.append("kpiClear.addEventListener(\"click\",function(){");
            html.append("kpiApplyRange(0);");
            html.append("var msg=kpiUpdateState();");
            html.append("if(msg){return;}");
            html.append("fetchKpiData(kpiParams());");
            html.append("});");
            ...
            html.append("var syncClear=document.getElementById(\"sync-clear\");");
            html.append("var syncRangeToday=document.getElementById(\"sync-range-today\");");
            html.append("var syncRange7=document.getElementById(\"sync-range-7\");");
            html.append("var syncRange30=document.getElementById(\"sync-range-30\");");
            html.append("function syncParams(){");
            html.append("return {");
            html.append("requestId:document.getElementById(\"sync-request-id\").value,");
            html.append("agency:document.getElementById(\"sync-agency\").value,");
            html.append("from:document.getElementById(\"sync-from\").value,");
            html.append("to:document.getElementById(\"sync-to\").value,");
            html.append("status:document.getElementById(\"sync-status\").value,");
            html.append("keyword:document.getElementById(\"sync-keyword\").value");
            html.append("};");
            html.append("}");
            html.append("function syncValidate(){");
            html.append("var params=syncParams();");
            html.append("var from=params.from;");
            html.append("var to=params.to;");
            html.append("var status=params.status;");
            ...
            html.append("}");
            html.append("function syncSetResultSummary(count){");
            html.append("var el=document.getElementById(\"sync-result-summary\");");
            html.append("if(!el){return;}");
            html.append("if(count===undefined||count===null){el.textContent=\"조회 결과: -\";return;}");
            html.append("el.textContent=\"조회 결과: \"+count+\"건\";");
            html.append("}");
            ...
            html.append("setDisabled(\"sync-search\",!!msg);");
            html.append("setInvalid([\"sync-from\",\"sync-to\",\"sync-status\"],!!msg);");
            html.append("setSummary(\"sync-summary\",syncParams());");
            html.append("return msg;");
            html.append("}");
            html.append("function formatDate(date){");
            html.append("var y=date.getFullYear();");
            html.append("var m=String(date.getMonth()+1).padStart(2,\"0\");");
            html.append("var d=String(date.getDate()).padStart(2,\"0\");");
            html.append("return y+\"-\"+m+\"-\"+d;");
            html.append("}");
            html.append("function applyRange(days){");
            html.append("var end=new Date();");
            html.append("var start=new Date();");
            html.append("start.setDate(end.getDate()-days+1);");
            html.append("document.getElementById(\"sync-from\").value=formatDate(start);");
            html.append("document.getElementById(\"sync-to\").value=formatDate(end);");
            html.append("syncUpdateState();");
            html.append("}");
            html.append("function resetRange(){");
            ...
            html.append("var el=document.getElementById(id);");
            html.append("if(!el){return;}");
            html.append("el.addEventListener(\"input\",syncUpdateState);");
            html.append("el.addEventListener(\"change\",syncUpdateState);");
            html.append("});");
            html.append("if(syncRangeToday){syncRangeToday.addEventListener(\"click\",function(){applyRange(1);});}");
            html.append("if(syncRange7){syncRange7.addEventListener(\"click\",function(){applyRange(7);});}");
            html.append("if(syncRange30){syncRange30.addEventListener(\"click\",function(){applyRange(30);});}");
            html.append("if(syncClear){syncClear.addEventListener(\"click\",resetRange);}");
            html.append("if(syncReset){");
            html.append("syncReset.addEventListener(\"click\",function(){");
            html.append("syncInputs.forEach(function(id){");
            html.append("var el=document.getElementById(id);");
            html.append("if(el){el.value=\"\";}");
            html.append("});");
            html.append("syncSetWarning(\"\");");
            html.append("syncSetResultSummary(null);");
            html.append("syncUpdateState();");
            html.append("document.getElementById(\"sync-log-body\").innerHTML=\"<tr><td colspan=\\\"5\\\" style=\\\"padding:4px 0;\\\">조회 대기</td></tr>\";");
            html.append("});");
            html.append("}");
